    ...
    if (window.electron) {
        window.electron.onLoadWheelOptions((wheelOptions) => {
            if (!wheelInitialized) {
                ...
                // Ensure we have an array (main may send array or wrapped object)
                const optionsArray = Array.isArray(wheelOptions) ? wheelOptions : (wheelOptions?.wheelOptions ?? wheelOptions ? [wheelOptions] : []);
                const enabledOptions = optionsArray.filter(opt => {
