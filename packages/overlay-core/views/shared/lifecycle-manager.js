...
class SharedQueueManager {
    ...
    spawnWorker(queueName) {
        ...
        // When core is in node_modules/overlay-core/views/shared, app root is ../../..; integration app sets OVERLAY_APP_ROOT
        const appRoot = process.env.OVERLAY_APP_ROOT || path.resolve(pathMod.join(__dirname, '..', '..', '..'));
        const worker = spawn(nodeExec, [workerPath, queueName], {
            stdio: ['ignore', 'inherit', 'inherit', 'ipc'],
            env: { ...process.env, OVERLAY_APP_ROOT: appRoot }
        });
